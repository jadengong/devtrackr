...
def demo_search_features():
    ...
    complex_examples = [
        ("High priority backend tasks", "GET /tasks/search?q=backend&priority=high&category=backend"),
        ("Urgent todo items", "GET /tasks/search?q=&status=todo&priority=urgent"),
        ("Frontend tasks due this week", "GET /tasks/search?q=frontend&category=frontend&due_after=2024-01-15&due_before=2024-01-21"),
        ("Completed authentication tasks", "GET /tasks/search?q=authentication&status=done")
    ]
...
def demo_search_implementation():
    ...
    print("SELECT t.*, ts_rank(to_tsvector('english', t.title || ' ' || COALESCE(t.description, '')),")
