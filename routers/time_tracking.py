...
from schemas import (
    ...
    TimerStart, TimerStop, ActiveTimer, TimeSummary
)
...
def stop_timer(
    ...
    if time_entry.status != TimeEntryStatus.active:
        raise HTTPException(
            status_code=400, detail="Timer is not active"
        )
...
def list_time_entries(
    ...
    time_entries = db.execute(
        stmt.order_by(TimeEntry.start_time.desc()).limit(limit)
    ).scalars().all()
...
def get_time_summary(
    ...
    category_stats = db.execute(
        ...
        ).group_by(TimeEntry.category).order_by(
...
def delete_time_entry(
    ...
    if (time_entry.status == TimeEntryStatus.completed and 
        time_entry.duration_minutes and 
        time_entry.task.actual_minutes):
        time_entry.task.actual_minutes = max(0, 
            time_entry.task.actual_minutes - time_entry.duration_minutes
        )
        db.add(time_entry.task)
