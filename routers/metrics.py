    ...
    for week_offset in range(weeks):
        ...
        if week_tasks.tasks_completed and week_tasks.tasks_completed > 0:
            ...
            if week_tasks.total_time_spent and week_tasks.total_time_spent > 0:
                ...
                time_score = min(50, (week_tasks.tasks_completed * 60) / week_tasks.total_time_spent * 25)
...
def get_time_efficiency(
    ...
    accuracy_percentage = (1 - abs(avg_estimated - avg_actual) / avg_estimated) * 100 if avg_estimated > 0 else 0
    ...
    return {
        ...
        "efficiency_ratio": round(efficiency_ratio, 2),  # >1 = overestimated, <1 = underestimated
...
def get_productivity_trends(
    ...
    if len(daily_data) >= 2:
        first_week_avg = sum(d['tasks_completed'] for d in daily_data[:7]) / min(7, len(daily_data))
        last_week_avg = sum(d['tasks_completed'] for d in daily_data[-7:]) / min(7, len(daily_data))
    ...
    return {
        ...
        "trend_analysis": {
            ...
            "first_week_average": round(first_week_avg, 2) if len(daily_data) >= 2 else 0,
            "last_week_average": round(last_week_avg, 2) if len(daily_data) >= 2 else 0
        }
    }
